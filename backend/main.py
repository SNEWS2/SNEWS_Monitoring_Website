...
from database import get_db, SigTierArchive, TimeTierArchive, CoincidenceTierArchive, HeartbeatArchive
    ...
    return [
        {
            ...
            "neutrino_time_utc": msg.neutrino_time_utc,
            "is_test": bool(msg.is_test),
            **{k: getattr(msg, k) for k in dir(msg) if not k.startswith('_') and k not in [
                'id', 'message_id', 'message_uuid', 'received_time_utc', 
                'detector_name', 'machine_time_utc', 'neutrino_time_utc', 'is_test'
            ]}
        }
        for msg in messages
    ]
...
def get_heartbeat_messages(db: Session = Depends(get_db)):
    return get_latest_messages(db, HeartbeatArchive)
